...
b=colnames(mat1)
     ...
     outmat = mat1$framewise_displacement
      ...
      outmat[is.na(outmat)]=0
      outmat=as.factor(outmat)
      ## CPU 03/03/2021 - output fd file as well as rms file
      ## BEGIN
      newout <- paste0(unlist(strsplit(out,"relRMS"))[1],"fd.1D",NULL)
      write.table(outmat,file=newout, col.names = F, row.names=F,quote=F)
      ## END
      } else  {
